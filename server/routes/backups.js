...
router.post('/', (req, res) => {
  try {
    ...
    execSync(
      `mysqldump -h ${shellQuote(dbHost)} -P ${shellQuote(dbPort)} -u ${shellQuote(dbUser)} -p${shellQuote(dbPass)} --single-transaction --quick --no-tablespaces ${shellQuote(dbName)} | gzip > ${shellQuote(filepath)}`,
      { timeout: 120000, shell: '/bin/bash' },
    );

    if (!fs.existsSync(filepath) || fs.statSync(filepath).size < 50) {
      return res.status(500).json({ error: 'Backup failed: empty dump' });
    }
...
router.post('/:filename/upload-drive', (req, res) => {
  try {
    ...
    if (!fs.existsSync(filepath)) return res.status(404).json({ error: 'Not found' });
    execSync(
      `rclone copy ${shellQuote(filepath)} gdrive:SMTradeBackups/ --config /root/.config/rclone/rclone.conf`,
      { timeout: 120000 },
    );
